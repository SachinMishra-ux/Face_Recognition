...
'''
def classify_image(pil_image_data, file_path=None):

    imgs = get_cropped_image_if_2_eyes(file_path, pil_image_data)
...
'''
def get_cropped_image_if_2_eyes(image_path, pil_image_data):
    ...
    gray = cv2.cvtColor(pil_image_data, cv2.COLOR_BGR2GRAY)
    faces = face_cascade.detectMultiScale(gray, 1.3, 5)

    cropped_faces = []
    for (x,y,w,h) in faces:
            roi_gray = gray[y:y+h, x:x+w]
            roi_color = pil_image_data[y:y+h, x:x+w]
